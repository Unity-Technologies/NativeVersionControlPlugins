...
static void add_child48(art_node48 *n, art_node **ref, unsigned char c, void *child) {
    if (n->n.num_children < 48) {
        ...
        art_node256 *newp = (art_node256*)alloc_node(NODE256);
        for (int i=0;i<256;i++) {
            if (n->keys[i]) {
                newp->children[i] = n->children[n->keys[i] - 1];
            }
        }
        copy_header((art_node*)newp, (art_node*)n);
        *ref = (art_node*)newp;
        free(n);
        add_child256(newp, ref, c, child);
    }
}
...
static void add_child16(art_node16 *n, art_node **ref, unsigned char c, void *child) {
    if (n->n.num_children < 16) {
        ...
        art_node48 *newp = (art_node48*)alloc_node(NODE48);

        // Copy the child pointers and populate the key map
        memcpy(newp->children, n->children,
                sizeof(void*)*n->n.num_children);
        for (int i=0;i<n->n.num_children;i++) {
            newp->keys[n->keys[i]] = i + 1;
        }
        copy_header((art_node*)newp, (art_node*)n);
        *ref = (art_node*)newp;
        free(n);
        add_child48(newp, ref, c, child);
    }
}
...
static void add_child4(art_node4 *n, art_node **ref, unsigned char c, void *child) {
    if (n->n.num_children < 4) {
        ...
        art_node16 *newp = (art_node16*)alloc_node(NODE16);

        // Copy the child pointers and the key map
        memcpy(newp->children, n->children,
                sizeof(void*)*n->n.num_children);
        memcpy(newp->keys, n->keys,
                sizeof(unsigned char)*n->n.num_children);
        copy_header((art_node*)newp, (art_node*)n);
        *ref = (art_node*)newp;
        free(n);
        add_child16(newp, ref, c, child);
    }
}
...
static void* recursive_insert(art_node *n, art_node **ref, char *key, int key_len, void* value, int depth, int *old) {
    ...
    if (IS_LEAF(n)) {
        ...
        *ref = (art_node*)newp;
        ...
        add_child4(newp, ref, l2->key[depth+longest_prefix], SET_LEAF(l2));
        return NULL;
    }
    ...
    if (n->partial_len) {
        ...
        *ref = (art_node*)newp;
        ...
        add_child4(newp, ref, key[depth+prefix_diff], SET_LEAF(l));
        return NULL;
    }
...
static void remove_child256(art_node256 *n, art_node **ref, unsigned char c) {
    ...
    if (n->n.num_children == 37) {
        ...
        *ref = (art_node*)newp;
        ...
        for (int i=0;i<256;i++) {
            if (n->children[i]) {
                newp->children[pos] = n->children[i];
                newp->keys[i] = pos + 1;
                pos++;
            }
        }
    ...
    if (n->n.num_children == 12) {
        ...
        *ref = (art_node*)newp;
        ...
        for (int i=0;i<256;i++) {
            ...
            if (pos) {
                newp->keys[child] = i;
                newp->children[child] = n->children[pos - 1];
                child++;
            }
        }
        free(n);
...
static void remove_child16(art_node16 *n, art_node **ref, art_node **l) {
    ...
    if (n->n.num_children == 3) {
        ...
        *ref = (art_node*)newp;
        copy_header((art_node*)newp, (art_node*)n);
        memcpy(newp->keys, n->keys, 4);
        memcpy(newp->children, n->children, 4*sizeof(void*));
        free(n);
    }
}
